  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <Link href="/configure" className="rounded-[2px] bg-ink px-5 py-2.5 text-[.72rem] font-semibold uppercase tracking-[.06em] text-white hover:bg-black">Request a new quote</Link>
      </div>

      {quotes.length === 0 ? (
        <div className="mt-7 rounded-lg border border-line bg-white p-8 text-center">
          <p className="text-ink-2">You have no quotes yet.</p>
          <Link href="/configure" className="mt-3 inline-block text-[.8rem] font-semibold uppercase tracking-[.06em] underline-offset-4 hover:underline">Configure a vehicle to request one</Link>
        </div>
      ) : (
