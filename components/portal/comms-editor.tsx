    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <div className="min-w-0 space-y-4">
        <div className="rounded-lg border border-line bg-white p-4">
          <label htmlFor="tpl-subject" className="block text-[.7rem] font-semibold uppercase tracking-[.08em] text-ink-2">Subject line</label>
          <input id="tpl-subject" aria-label="Subject line" value={subject} onChange={(e) => setSubject(e.target.value)} className="mt-1.5 w-full rounded-md border border-line-2 px-3 py-2 text-[.9rem] focus:border-ink focus:outline-none" />
          <label htmlFor="tpl-preheader" className="mt-3 block text-[.7rem] font-semibold uppercase tracking-[.08em] text-ink-2">Preheader <span className="font-normal normal-case text-ink-3">(inbox preview text)</span></label>
          <input id="tpl-preheader" aria-label="Preheader" value={preheader} onChange={(e) => setPreheader(e.target.value)} className="mt-1.5 w-full rounded-md border border-line-2 px-3 py-2 text-[.9rem] focus:border-ink focus:outline-none" />
        </div>
        ...
        <div className="rounded-lg border border-line bg-white">
          ...
          {view === "preview" ? (
            ...
            <textarea ref={htmlRef} aria-label="Email HTML source" value={html} onChange={(e) => setHtml(e.target.value)} spellCheck={false} className="h-[620px] w-full resize-none rounded-b-lg bg-[#0d1117] p-4 font-mono text-[.78rem] leading-relaxed text-[#c9d1d9] focus:outline-none" />
          )}
        </div>
