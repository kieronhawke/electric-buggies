...
/** Horizontal progress tracker. Near-black for done/current, hairline for upcoming. */
        ...
        return (
          <li key={step.key} className="relative flex flex-1 flex-col items-center">
            ...
            {i > 0 && (
              <span
                aria-hidden
                className={cn("absolute right-1/2 top-[11px] h-[2px] w-full", i <= current ? "bg-ink" : "bg-line-2")}
              />
            )}
            <span
              aria-hidden
              className={cn(
                ...
                !done && !isCurrent && "border-line-2 bg-white text-ink-2",
              )}
            >
              {done ? (
                ...
                <span className={cn("h-[7px] w-[7px] rounded-full", isCurrent ? "bg-ink" : "bg-line-2")} />
              )}
            </span>
            <span className={cn("mt-2 w-full truncate px-0.5 text-center text-[.5rem] font-semibold uppercase leading-tight tracking-[.01em] sm:text-[.68rem] sm:tracking-[.04em]", i <= current ? "text-ink" : "text-ink-2")}>
              {step.label}
            </span>
          </li>
        );
      ...
      {events.map((e, i) => {
        ...
        return (
          <li key={e.id} className="relative flex gap-4 pb-6 last:pb-0">
            ...
            <span aria-hidden className="relative z-10 mt-1.5 h-[13px] w-[13px] flex-none rounded-full border-2 border-ink bg-ink" />
